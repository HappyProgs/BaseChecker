...
function startSearch() {
  ...
  // ⬇️ Добавляем разбиение по запятым
  const queries = query.split(",").map(t => t.trim()).filter(Boolean);
  ...
  // ⬇️ Запускаем поиск для каждого терма
  queries.forEach(q => {
    controller.start({
  ...
  const data = window.searchResults.join("\n");
